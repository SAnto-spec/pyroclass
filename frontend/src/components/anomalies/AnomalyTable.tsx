  ...
  return (
    <div className="overflow-hidden rounded-md border border-slate-800 bg-slate-900">
          ...
          <tbody className="divide-y divide-slate-800">
            {anomalies.map((a) => {
              ...
              return (
                ...
                >
                  <td className="whitespace-nowrap px-3 py-2.5 font-mono text-xs font-medium text-slate-200">
                    {a.id}
                  </td>

                  <td className="whitespace-nowrap px-3 py-2.5 tabular-nums text-slate-400">
                  ...
                  <td className="whitespace-nowrap px-3 py-2.5">
                    <span
                      className={[
                        ...
                        a.persistenceScore >= 70
                          ? "bg-amber-500/15 text-amber-400"
                          : a.persistenceScore >= 40
                            ? "bg-slate-800 text-slate-400"
                            : "bg-slate-800 text-slate-500",
                      ].join(" ")}
                    >
                      {a.persistenceScore.toFixed(0)}%
                    </span>
                  </td>

                  <td className="whitespace-nowrap px-3 py-2.5 text-slate-400">
                    {a.nearbyFacility
                      ? a.nearbyFacility.distanceKm > 0
                        ? `${a.nearbyFacility.name} · ${a.nearbyFacility.distanceKm.toFixed(1)} km`
      ...
      <div className="border-t border-slate-800 bg-slate-950/30 px-3 py-2 text-[11px] text-slate-500">
        {anomalies.length}{" "}
        {anomalies.length === 1 ? "result" : "results"} · Click a row for
        details
      </div>
    </div>
  );
