  ...
  return (
    <section role="region" aria-label="Geospatial map" className={className ?? "relative flex h-[380px] flex-col overflow-hidden rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] shadow-[var(--shadow-sm)] sm:h-[440px] lg:h-[560px]"}>
      ...
      <div className="absolute left-2 top-10 z-[1] rounded-[var(--radius-md)] border border-[var(--border)] bg-white px-2.5 py-2 text-xs shadow-[var(--shadow-md)]">
        ...
        <label className="flex cursor-pointer items-center gap-2 py-1 text-[var(--text-secondary)]"><input type="checkbox" checked={showAnomalies} onChange={(e) => setShowAnomalies(e.target.checked)} className="h-3 w-3 rounded border-[var(--border-strong)] bg-white text-[var(--accent)] focus:ring-[var(--accent)]" /><span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-[var(--accent)] border border-white shadow-sm" /> Thermal Anomalies</span></label>
        <label className="flex cursor-pointer items-center gap-2 py-1 text-[var(--text-secondary)]"><input type="checkbox" checked={showFacilities} onChange={(e) => setShowFacilities(e.target.checked)} className="h-3 w-3 rounded border-[var(--border-strong)] bg-white text-amber-600 focus:ring-amber-600" /><span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-slate-700 border border-amber-600 shadow-sm" /> Industrial Facilities</span></label>
        <label className="flex cursor-pointer items-center gap-2 py-1 text-[var(--text-secondary)]"><input type="checkbox" checked={showSources} onChange={(e) => setShowSources(e.target.checked)} className="h-3 w-3 rounded border-[var(--border-strong)] bg-white text-[var(--accent)] focus:ring-[var(--accent)]" /><span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-[var(--accent)]/50 border border-white shadow-sm" /> Persistent Industrial Heat</span></label>
        <label className="flex cursor-pointer items-center gap-2 py-1 text-[var(--text-secondary)]"><input type="checkbox" checked={showCommunity} onChange={(e) => setShowCommunity(e.target.checked)} className="h-3 w-3 rounded border-[var(--border-strong)] bg-white text-[#0f5e59] focus:ring-[#0f5e59]" /><span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-[#0f5e59] border border-white shadow-sm" /> Ground Observations</span></label>
      </div>
    </section>
  );
}
